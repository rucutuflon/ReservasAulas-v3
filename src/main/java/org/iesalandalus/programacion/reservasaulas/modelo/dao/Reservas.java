...
public class Reservas implements Serializable {
	...
	public void insertar(Reserva reserva) throws OperationNotSupportedException{
		...
		if(!esMesSiguienteOPosterior(reserva)) {
			throw new OperationNotSupportedException("S�lo se pueden hacer reservas para el mes que viene o posteriores.");
		}
		...
		if(aux != null && aux.getAula().equals(reserva.getAula())) {
			if(aux.getPermanencia() instanceof PermanenciaPorTramo && reserva.getPermanencia() instanceof PermanenciaPorTramo) {
				if(((PermanenciaPorTramo)aux.getPermanencia()).getTramo().equals(((PermanenciaPorTramo)reserva.getPermanencia()).getTramo())) {
					throw new OperationNotSupportedException("Ya se ha realizado una reserva por tramo para este d�a y aula.");
				}
			}else if(aux.getPermanencia() instanceof PermanenciaPorTramo) {
				throw new OperationNotSupportedException("Ya se ha realizado una reserva por tramo para este d�a y aula.");
			}else {
				throw new OperationNotSupportedException("Ya se ha realizado una reserva por hora para este d�a y aula.");
			}
		}
		...
		if(suma + reserva.getPuntos() > MAX_PUNTOS_PROFESOR_MES) {
			throw new OperationNotSupportedException("Esta reserva excede los puntos m�ximos por mes para dicho profesor.");
		}
